...
namespace PiGeChang
{
    class UsbSafelyRemove
    {
        ...
        static long GetDrivesDevInstByDeviceNumber(long DeviceNumber, DriveType DriveType, string dosDeviceName)
        {
            ...
            if (hDevInfo == new IntPtr(INVALID_HANDLE_VALUE)) throw new Win32Exception(Marshal.GetLastWin32Error());
            ...
            while (true)
            {
                ...
                // Set according to https://stackoverflow.com/questions/10728644/properly-declare-sp-device-interface-detail-data-for-pinvoke
                // Who knows a better way to set cbSize?
                detailData.cbSize = IntPtr.Size == 4 ? (sizeof(int) + Marshal.SystemDefaultCharSize) : 8;
                ...
                IntPtr pDevicePath = buffer + sizeof(int);
